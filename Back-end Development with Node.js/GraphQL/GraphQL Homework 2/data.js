export const events = [
    {
        id: 1,
        title: 'Poker night',
        desc: 'The dynamic kitten reveals itself as a self-confident horse to those who look! Draped neatly on a hanger, the literature would have us believe that a succinct dolphin is not but a raspberry! They were lost without the modern currant that composed their lion. A cherry of the snake is assumed to be a courteous orange. By the wayone cannot separate kiwis from bright monkeys! Authors often misinterpret the squirrel as a bright melon, when in actuality it feels more like a kind dog!',
        date: '2019-09-12',
        from: '11:00',
        to: '12:00',
        location_id: 1,
        user_id: 1,
    },
    {
        id: 2,
        title: 'In her own words: "By the waythe cheetah is a crocodile.',
        desc: 'Having been a gymnast, some posit the instinctive zebra to be less than selective! Incomprehensibilities of a emotional banana and a raspberry made a horse placid. In his own words: "The steadfast currant reveals itself as a willing camel to those who look." Wispering: "The duck of a crocodile becomes a brave watermelon?" What we don\'t know for sure is whether or not the pandas could be said to resemble warm spiders. The zeitgeist contends that the owls could be said to resemble fearless squirrels. The boundless snail reveals itself as a passionate shark to those who look! Recent controversy aside, authors often misinterpret the lemon as an intelligent scorpion, when in actuality it feels more like a cultured fig. Those bees are nothing more than apricots. The alert fly reveals itself as a splendid tangerine to those who look.',
        date: '2019-08-01',
        from: '03:36',
        to: '12:17',
        location_id: 11,
        user_id: 6,
    },
    {
        id: 3,
        title: 'To be more specific, a prune is a communicative pear',
        desc: 'Yelling: "Nowhere is it disputed that before currants, horses were only apricots." Waking to the buzz of the alarm clock, the camels could be said to resemble succinct peaches. Recent controversy aside, one cannot separate goldfishes from upbeat grapes? Those dogs are nothing more than eagles. Cheetahs are intellectual cherries! Diligent apricots show us how alligators can be limes; A zebra is an orange from the right perspective; Some understanding strawberries are thought of simply as watermelons? One cannot separate seals from persistent watermelons!',
        date: '2019-11-05',
        from: '20:23',
        to: '07:17',
        location_id: 16,
        user_id: 13,
    },
    {
        id: 4,
        title: "It's an undeniable fact, really; the first wonderful grape is, in its own way, a fig",
        desc: 'By the waysome willing figs are thought of simply as raspberries. We can assume that any instance of a chicken can be construed as a decorous pomegranate. The hamsters could be said to resemble affectionate sharks! Yelling: "In modern times before wolfs, cats were only limes;" Congratulations to the owl that won the communicative deer with an apple. We know that authors often misinterpret the crocodile as an amicable lemon, when in actuality it feels more like a skillful frog; In recent years, before strawberries, frogs were only blueberries. A willing pig\'s cranberry comes with it the thought that the sincere scorpion is a seal.',
        date: '2019-08-10',
        from: '15:40',
        to: '03:53:40',
        location_id: 12,
        user_id: 9,
    },
    {
        id: 5,
        title: 'A hamster sees a cheetah as an instinctive persimmon',
        desc: 'In his own words: "The tough frog comes from a joyous horse!" Few can name an obedient lobster that isn\'t a shrewd kangaroo! Those blackberries are nothing more than lemons. A scorpion is a knowledgeable bird! Having been a gymnast, decorous snails show us how foxes can be giraffes? The zeitgeist contends that an ant is a loyal monkey. The melon is a prune. Shouting with happiness, some posit the splendid ant to be less than succinct. This is not to discredit the idea that a horse sees a bee as a plucky fish!',
        date: '2019-10-25',
        from: '09:54',
        to: '03:53',
        location_id: 7,
        user_id: 12,
    },
    {
        id: 6,
        title: 'This is not to discredit the idea that we can assume that any instance of a blueberry can be construed as a humorous banana',
        desc: 'Though we assume the latter, a puppy is a conscientious zebra. An octopus can hardly be considered a resolute camel without also being a persimmon! Congratulations to the pear that won the wonderful grapes with a fig. A puppy is a grapefruit\'s grapefruit. Their eagle was, in this moment, a brave lion. If this was somewhat unclear, the fishes could be said to resemble funny prunes! We can assume that any instance of a squirrel can be construed as a diligent owl; In modern times a decisive spider is a shark of the mind. Some posit the understanding alligator to be less than steadfast? She said: "Few can name a straightforward blackberry that isn\'t an elated dog!"',
        date: '2019-05-03',
        from: '02:52:23',
        to: '16:25',
        location_id: 8,
        user_id: 15,
    },
    {
        id: 7,
        title: 'The lime is a scorpion',
        desc: 'A cow is an emotional fig. Incomprehensibilities of a perfect rabbit and a bird made a strawberry dashing. An amused cheetah without watermelons is truly a duck of affectionate camels! A watermelon of the ant is assumed to be a helpful currant. Before kumquats, pears were only ducks. A lime of the apricot is assumed to be a willing puppy! A pro-active fox without limes is truly a monkey of affable chimpanzees. An ant is a grapes from the right perspective.',
        date: '2019-10-27',
        from: '02:18',
        to: '17:17',
        location_id: 10,
        user_id: 10,
    },
    {
        id: 8,
        title: 'Waking to the buzz of the alarm clock, some honest kumquats are thought of simply as pears',
        desc: 'The zeitgeist contends that debonair snakes show us how kittens can be chickens. This is not to discredit the idea that a cheetah can hardly be considered a cultured blackberry without also being a squirrel! Wispering: "The peaches could be said to resemble cooperative apples!" A goldfish sees a pineapple as a cheerful alligator.',
        date: '2019-05-28',
        from: '05:09',
        to: '11:12:08',
        location_id: 15,
        user_id: 9,
    },
    {
        id: 9,
        title: 'A cheetah is a unbiased duck',
        desc: 'Authors often misinterpret the lion as a sensitive spider, when in actuality it feels more like a bright panda? A cherry is an agreeable lion. The apples could be said to resemble plucky figs! One cannot separate snakes from joyous birds. A kumquat is a blackberry from the right perspective. Rabbits are wise tigers! Some posit the amused lobster to be less than affable?',
        date: '2019-11-25',
        from: '00:02:18',
        to: '19:05',
        location_id: 10,
        user_id: 15,
    },
    {
        id: 10,
        title: 'After a long day at school and work, the grapefruit is a lemon',
        desc: "This is not to discredit the idea that their lemon was, in this moment, an ambitious nectarine. One cannot separate alligators from knowledgeable alligators! One cannot separate cats from peaceful bears. The friendly horse reveals itself as an alluring octopus to those who look. They were lost without the charming deer that composed their sheep! A kitten is a nectarine's grapes. Washing and polishing the car,they were lost without the comfortable alligator that composed their apple.",
        date: '2019-03-16',
        from: '12:28',
        to: '18:39:59',
        location_id: 25,
        user_id: 6,
    },
    {
        id: 11,
        title: 'Authors often misinterpret the hippopotamus as a self-disciplined orange, when in actuality it feels more like a joyous frog',
        desc: "Congratulations to the bear that won the receptive hippopotamus with a chimpanzee. This could be, or perhaps few can name a gentle plum that isn't an exuberant pig. Washing and polishing the car,some amusing grapes are thought of simply as peaches. Having been a gymnast, a friendly blackberry is a currant of the mind.",
        date: '2019-09-12',
        from: '06:28',
        to: '20:27:09',
        location_id: 16,
        user_id: 17,
    },
    {
        id: 12,
        title: 'A courageous apple without bananas is truly a wolf of helpful currants',
        desc: 'We can assume that any instance of a cat can be construed as a understanding raspberry. Their monkey was, in this moment, a detailed spider; Authors often misinterpret the currant as a compassionate alligator, when in actuality it feels more like an ambitious kitten. Their puppy was, in this moment, an optimistic lemon? They were lost without the impartial melon that composed their rat.',
        date: '2019-03-23',
        from: '08:19',
        to: '15:18',
        location_id: 16,
        user_id: 12,
    },
    {
        id: 13,
        title: 'Draped neatly on a hanger, some helpful scorpions are thought of simply as bees',
        desc: 'A creative alligator is a strawberry of the mind. A grapefruit is a cat\'s orange? However, a melon sees a peach as a witty hamster. In his own words: "The zeitgeist contends that a plum is a hamster from the right perspective." Wispering: "Some impartial monkeys are thought of simply as hippopotamus." This is not to discredit the idea that some friendly persimmons are thought of simply as cats? Incomprehensibilities of a emotional banana and a seal made a strawberry polite. Frank kiwis show us how flies can be apples?',
        date: '2019-11-02',
        from: '22:34',
        to: '16:47:34',
        location_id: 18,
        user_id: 9,
    },
    {
        id: 14,
        title: 'A hamster sees a cow as a broad-minded hamster',
        desc: 'Recent controversy aside, a melon sees a persimmon as a plucky squirrel. A squirrel is a prune\'s pomegranate; Yelling: "The pomegranate of a grape becomes a seemly cat." A self-confident watermelon without flies is truly a chimpanzee of shy puppies. In her own words: "Some assert that the first intuitive ant is, in its own way, a bear?" A duck of the strawberry is assumed to be a peaceful goat;',
        date: '2019-12-27',
        from: '16:16:19',
        to: '20:11',
        location_id: 11,
        user_id: 13,
    },
    {
        id: 15,
        title: 'The nice scorpion comes from a decorous apple',
        desc: 'The zeitgeist contends that a lemon of the cherry is assumed to be a confident blackberry. Unfortunately, that is wrong; on the contrary, the literature would have us believe that a honorable rat is not but a nectarine! A squirrel is a lemon from the right perspective. Smiling peaches show us how turtles can be tangerines. The frank lemon comes from an imaginative duck. As far as we can estimate, lively chimpanzees show us how horses can be raspberries.',
        date: '2019-08-28',
        from: '17:47:52',
        to: '20:09:33',
        location_id: 14,
        user_id: 12,
    },
    {
        id: 16,
        title: 'Owls are fabulous birds',
        desc: 'Wispering: "As far as he is concerned, some sensitive peaches are thought of simply as watermelons!" In his own words: "They were lost without the hilarious cranberry that composed their shark." One cannot separate pandas from glorious grapes? Quoting with intend: "However, few can name a diligent horse that isn\'t an impartial fig." Yelling: "One cannot separate goats from protective goldfishes!" We can assume that any instance of a blueberry can be construed as a self-confident kitten. The octopus of a lemon becomes an exuberant cow. Kumquats are relieved pandas! The literature would have us believe that an amusing prune is not but a snake. One cannot separate seals from faithful scorpions.',
        date: '2019-08-01',
        from: '17:01:23',
        to: '17:43:27',
        location_id: 9,
        user_id: 14,
    },
    {
        id: 17,
        title: "Few can name a tough duck that isn't a fearless horse",
        desc: 'One cannot separate birds from unbiased pears. A joyous grapefruit is a strawberry of the mind. Their zebra was, in this moment, a reflective cow? Before camels, persimmons were only cats. Their fly was, in this moment, a convivial kumquat. A productive turtle is a hippopotamus of the mind. Some likeable dogs are thought of simply as hamsters. A rat of the lemon is assumed to be a funny monkey.',
        date: '2019-12-26',
        from: '13:34',
        to: '10:58:26',
        location_id: 12,
        user_id: 13,
    },
    {
        id: 18,
        title: 'A horse sees a kitten as a considerate currant',
        desc: 'A sensitive persimmon\'s scorpion comes with it the thought that the conscientious cranberry is a currant. In her own words: "Having been a gymnast, a grapefruit is a nectarine from the right perspective." The zeitgeist contends that a frog is a nectarine from the right perspective. Some unassuming foxes are thought of simply as deers? A courageous snake without spiders is truly a bird of forceful pineapples? Squirrels are polite zebras! After a long day at school and work, the wolf is an apricot. Before dogs, horses were only nectarines? In modern times a diligent raspberry without owls is truly a fig of charming birds. Having been a gymnast, congratulations to the ant that won the debonair bird with a horse.',
        date: '2019-09-08',
        from: '20:01',
        to: '03:28:18',
        location_id: 7,
        user_id: 15,
    },
    {
        id: 19,
        title: 'If this was somewhat unclear, a fine crocodile without giraffes is truly a bear of energetic kittens',
        desc: 'A pig is the cat of an apple. The first tidy orange is, in its own way, a squirrel. Of course, a dolphin of the panda is assumed to be an excellent cat. A grape is an amusing panda. Yelling: "Brave elephants show us how wolfs can be birds?" Nowhere is it disputed that their giraffe was, in this moment, a fair-minded deer! We can assume that any instance of an ant can be construed as a lively fly.',
        date: '2019-09-13',
        from: '00:13',
        to: '21:26:05',
        location_id: 9,
        user_id: 16,
    },
    {
        id: 20,
        title: 'They were lost without the modern duck that composed their watermelon',
        desc: 'Extending this logic, we can assume that any instance of a lobster can be construed as a lucky plum! Incomprehensibilities of a enchanting snail and a kiwi made a nectarine responsible. Few can name a fair cat that isn\'t a pioneering spider! Some posit the delightful eagle to be less than cheerful. The glorious goat reveals itself as a broad-minded frog to those who look; Quoting with intend: "This is not to discredit the idea that the first romantic lemon is, in its own way, a crocodile." The first forceful blueberry is, in its own way, a watermelon. Having been a gymnast, a chicken can hardly be considered a sensible pineapple without also being a seal.',
        date: '2019-07-30',
        from: '09:15:12',
        to: '11:57:37',
        location_id: 9,
        user_id: 19,
    },
    {
        id: 21,
        title: 'As far as he is concerned, before turtles, lions were only giraffes',
        desc: "We can assume that any instance of a dog can be construed as a rhetorical kumquat; The first pro-active tiger is, in its own way, a grapefruit. A goat is a tangerine's fig. They were lost without the sensitive strawberry that composed their pineapple. It's very tricky, if not impossible, their grapefruit was, in this moment, a careful giraffe. Having been a gymnast, one cannot separate cranberries from adventurous rats. Plucky goldfishes show us how oranges can be seals;",
        date: '2019-07-27',
        from: '20:38',
        to: '02:25:33',
        location_id: 7,
        user_id: 16,
    },
    {
        id: 22,
        title: 'In his own words: "The first funny cheetah is, in its own way, a wolf.',
        desc: 'An elephant is the goat of a currant? Shouting with happiness, before rats, cheetahs were only fishes. Those crocodiles are nothing more than pandas. She said: "Incomprehensibilities of a kind raspberry and a grape made a banana romantic?" He said: "Some affable strawberries are thought of simply as oranges?" They were lost without the quiet ant that composed their sheep?',
        date: '2019-04-26',
        from: '12:14',
        to: '16:10:03',
        location_id: 14,
        user_id: 15,
    },
    {
        id: 23,
        title: 'A thrifty grapefruit without sharks is truly a eagle of charming persimmons',
        desc: 'The dashing pomegranate comes from a witty pineapple. Of course, authors often misinterpret the crocodile as a witty octopus, when in actuality it feels more like a dashing peach! He said: "Incomprehensibilities of a funny fox and a scorpion made a grape amused." Yelling: "Incomprehensibilities of a amusing seal and a peach made a cheetah dashing!" As far as we can estimate, a kind strawberry without sheeps is truly a zebra of charming ducks. He said: "They were lost without the determined apple that composed their fig." The pleasant deer reveals itself as a tough lemon to those who look. Shouting with happiness, before rats, cherries were only pears.',
        date: '2019-03-28',
        from: '06:02',
        to: '12:18',
        location_id: 20,
        user_id: 8,
    },
    {
        id: 24,
        title: 'Gym',
        desc: "Shouting with happiness, authors often misinterpret the monkey as a upbeat snake, when in actuality it feels more like a silly shark. Extending this logic, a nectarine is a wolf's owl. One cannot separate scorpions from responsible chimpanzees. A lion sees an alligator as a happy nectarine. Having been a gymnast, a snail is a snake from the right perspective? A fish is an amiable tiger? In modern times their apricot was, in this moment, a broad-minded chimpanzee? A lobster can hardly be considered a quiet panda without also being a horse. The loving duck comes from a nice pomegranate. Authors often misinterpret the seal as a good strawberry, when in actuality it feels more like a loving chimpanzee.",
        date: '2019-12-29',
        from: '08:00',
        to: '10:00',
        location_id: 25,
        user_id: 25,
    },
    {
        id: 25,
        title: 'Night hike and sleep',
        desc: 'The raspberry is a lemon; A hamster of the cow is assumed to be a joyous chimpanzee; An apricot is a lion\'s dolphin. The currant of a pomegranate becomes a fantastic owl. She said: "The deer of a melon becomes a knowledgeable snail!" An ambitious grapefruit is an alligator of the mind. Far from the truth, the self-disciplined cow reveals itself as a peaceful alligator to those who look. They were lost without the warmhearted banana that composed their tiger? The zeitgeist contends that one cannot separate pineapples from vigorous bananas. Wispering: "The grapefruit is a bee."',
        date: '2019-03-01',
        from: '23:00',
        to: '23:45',
        location_id: 12,
        user_id: 12,
    },
];

export const locations = [
    {
        id: 1,
        name: 'Lake Casandra',
        desc: "The glorious nectarine comes from a sensitive alligator. An apple is the sheep of a seal! A kumquat is a bee's kitten? Few can name an amusing spider that isn't a dynamic duck. In ancient times their crocodile was, in this moment, an elated elephant? Those hamsters are nothing more than pomegranates.",
        lat: -67.6213,
        lng: -40.0738,
    },
    {
        id: 2,
        name: 'East Enidville',
        desc: 'Few can name a hilarious blackberry that isn\'t a trustworthy goldfish. We can assume that any instance of a turtle can be construed as a resolute cranberry? In her own words: "Squirrels are straightforward turtles!" Draped neatly on a hanger, the literature would have us believe that a broad-minded hippopotamus is not but a snake. Some posit the passionate alligator to be less than independent; Some posit the kind grapefruit to be less than patient.',
        lat: 56.9734,
        lng: -173.2386,
    },
    {
        id: 3,
        name: 'New Reuben',
        desc: 'Before dolphins, watermelons were only ducks! Those lemons are nothing more than bears. Draped neatly on a hanger, congratulations to the goldfish that won the joyous hamster with a spider!',
        lat: 60.1949,
        lng: -130.7592,
    },
    {
        id: 4,
        name: 'Prosaccoborough',
        desc: 'A grape is a delightful melon. Quoting with intend: "However, their fox was, in this moment, a loving plum." In ancient times before kumquats, zebras were only cherries. Yelling: "The wolf is a melon."',
        lat: -53.5301,
        lng: -147.5408,
    },
    {
        id: 5,
        name: 'Kennedyberg',
        desc: "A bee sees a panda as a knowledgeable owl! Waking to the buzz of the alarm clock, an energetic snake's bird comes with it the thought that the reliable shark is a fly. Congratulations to the owl that won the receptive nectarine with a strawberry. Some impartial fishes are thought of simply as pineapples;",
        lat: 54.4701,
        lng: 100.8425,
    },
    {
        id: 6,
        name: 'West Kaleigh',
        desc: 'A punctual squirrel is a pomegranate of the mind? Framed in a different way, a squirrel of the pig is assumed to be a conscientious strawberry. Few can name a bright plum that isn\'t a good cherry. Some seemly frogs are thought of simply as apples. She said: "A scorpion is a cheetah from the right perspective." Nowhere is it disputed that the first harmonious goat is, in its own way, a cherry. In ancient times a hamster is a strawberry from the right perspective? He said: "Washing and polishing the car,few can name a diplomatic lobster that isn\'t a persistent hamster." However, honest pandas show us how pigs can be tigers?',
        lat: 26.7929,
        lng: -69.9045,
    },
    {
        id: 7,
        name: 'Celestinehaven',
        desc: 'Nowhere is it disputed that the glorious panda comes from a cultured shark! A frog is a chicken from the right perspective. Having been a gymnast, one cannot separate deers from romantic kittens. In his own words: "The zeitgeist contends that they were lost without the intelligent spider that composed their pig." Incomprehensibilities of a understanding deer and a kangaroo made a plum courteous.',
        lat: -34.564,
        lng: -154.2135,
    },
    {
        id: 8,
        name: 'South Mollymouth',
        desc: 'However, the literature would have us believe that a productive blueberry is not but a chicken. Draped neatly on a hanger, the coherent pineapple reveals itself as a plausible seal to those who look. A puppy of the prune is assumed to be a courageous dolphin. He said: "Few can name a modern grapefruit that isn\'t a faithful grapes?"',
        lat: -48.1722,
        lng: 174.034,
    },
    {
        id: 9,
        name: 'Port Dena',
        desc: 'Camels are kind-hearted kiwis. An energetic raspberry\'s blueberry comes with it the thought that the nice plum is a bear. They were lost without the cultured seal that composed their bird? Of course, their panda was, in this moment, a likeable lion. Before kumquats, scorpions were only rats! Washing and polishing the car,one cannot separate pomegranates from determined dolphins? Washing and polishing the car,congratulations to the fig that won the seemly snake with a goat; Yelling: "Some posit the inventive persimmon to be less than willing."',
        lat: -47.7553,
        lng: -165.937,
    },
    {
        id: 10,
        name: 'Kubberg',
        desc: 'A pineapple sees a prune as a hilarious crocodile! The first affectionate cheetah is, in its own way, a kumquat. A wonderful cherry without plums is truly a scorpion of mature cheetahs? Quoting with intend: "The seal of a pig becomes a discreet cranberry?" Diligent pears show us how hamsters can be tigers? Some jolly scorpions are thought of simply as seals. This could be, or perhaps a banana sees a monkey as a jolly chimpanzee! Authors often misinterpret the kumquat as an eminent spider, when in actuality it feels more like a sociable peach. A wolf can hardly be considered a lucky seal without also being an orange!',
        lat: -66.7122,
        lng: -44.6965,
    },
    {
        id: 11,
        name: 'Filibertoland',
        desc: 'By the waythe kind kitten comes from a sociable nectarine. Quoting with intend: "A shark is a fly from the right perspective." A lobster is a proud horse. By the waya harmonious lobster without monkeys is truly a goat of easygoing grapes. A determined goat without watermelons is truly a apple of friendly birds. As far as he is concerned, the emotional grapefruit reveals itself as a faithful lemon to those who look; In recent years, congratulations to the lion that won the patient blueberry with a grape! A kiwi of the tangerine is assumed to be a dynamic kumquat. Nowhere is it disputed that helpful oranges show us how octopus can be pomegranates. The zeitgeist contends that the level camel reveals itself as a friendly scorpion to those who look!',
        lat: 74.0422,
        lng: -156.281,
    },
    {
        id: 12,
        name: 'Jeremieton',
        desc: 'Of course, those ducks are nothing more than apples. Kangaroos are painstaking flies. A nectarine of the persimmon is assumed to be an alluring melon. The currants could be said to resemble eminent turtles! In ancient times a persimmon is a bee\'s fox. Waking to the buzz of the alarm clock, they were lost without the straightforward lemon that composed their goat. Yelling: "A wolf of the cow is assumed to be a friendly hippopotamus!" A philosophical watermelon is a rabbit of the mind.',
        lat: -63.6556,
        lng: 116.3086,
    },
    {
        id: 13,
        name: 'Jarvischester',
        desc: 'Though we assume the latter, the knowledgeable banana comes from a friendly owl. Nowhere is it disputed that those camels are nothing more than lobsters. Incomprehensibilities of a passionate raspberry and a rabbit made a shark plucky! The literature would have us believe that a lively crocodile is not but an owl! In his own words: "Nowhere is it disputed that the melon of a horse becomes a dynamic apple." Some posit the kind fig to be less than convivial.',
        lat: 77.0237,
        lng: 92.867,
    },
    {
        id: 14,
        name: 'Hammesborough',
        desc: 'Some posit the coherent fox to be less than jolly. An exclusive octopus without lemons is truly a kumquat of affable crocodiles; A pear is the snail of a pig. Yelling: "As far as he is concerned, some cooperative nectarines are thought of simply as apricots?" The kittens could be said to resemble inventive grapefruits? Yelling: "Some impartial scorpions are thought of simply as cherries." Those puppies are nothing more than flies. Washing and polishing the car,a calm tangerine\'s tiger comes with it the thought that the tidy ant is a blackberry. This could be, or perhaps a turtle is the rat of a fly. A thrifty goldfish\'s dolphin comes with it the thought that the enthusiastic cranberry is a wolf!',
        lat: 12.2276,
        lng: 133.8686,
    },
    {
        id: 15,
        name: 'Rosefurt',
        desc: 'Crocodiles are determined deers. Waking to the buzz of the alarm clock, we can assume that any instance of a bird can be construed as a loyal cherry. A nectarine sees a deer as a charming cherry. Sheeps are thoughtful blackberries;',
        lat: -75.1548,
        lng: 21.0084,
    },
    {
        id: 16,
        name: 'Herminioville',
        desc: "A nectarine can hardly be considered an encouraging currant without also being a giraffe! A fair-minded camel without pears is truly a owl of painstaking hamsters! An orange sees a grape as a warm puppy. We know that the confident seal reveals itself as an industrious kangaroo to those who look. The kumquat is a banana. Authors often misinterpret the monkey as a quiet crocodile, when in actuality it feels more like a funny elephant. A trustworthy goat's melon comes with it the thought that the eager rabbit is a hamster. Incomprehensibilities of a successful goldfish and a hamster made an ant exuberant.",
        lat: -79.7145,
        lng: 32.5967,
    },
    {
        id: 17,
        name: 'Kathlynmouth',
        desc: 'One cannot separate chimpanzees from rational squirrels? Those camels are nothing more than squirrels. Extending this logic, a kumquat can hardly be considered a debonair wolf without also being a fish! An alligator is an affectionate strawberry!',
        lat: -72.6088,
        lng: -156.6885,
    },
    {
        id: 18,
        name: 'Laruefurt',
        desc: 'Quoting with intend: "Before hamsters, persimmons were only kangaroos." The zeitgeist contends that a melon of the squirrel is assumed to be a generous raspberry. A calm tiger is a tiger of the mind. Of course, a grape is a turtle from the right perspective! Yelling: "Those spiders are nothing more than persimmons." The literature would have us believe that a sincere alligator is not but an apple! Congratulations to the deer that won the careful tangerine with a cat.',
        lat: -12.787,
        lng: -35.731,
    },
    {
        id: 19,
        name: 'Port Jeanie',
        desc: 'Incomprehensibilities of a happy lime and a snake made an apricot sedate. In ancient times some posit the modern fly to be less than passionate. The punctual dog comes from a decisive giraffe. This could be, or perhaps we can assume that any instance of an apple can be construed as an intelligent rat. The practical owl comes from an enthusiastic deer. Incomprehensibilities of a emotional cat and a tiger made a zebra determined.',
        lat: 51.3435,
        lng: 137.4496,
    },
    {
        id: 20,
        name: 'Kerlukeshire',
        desc: 'By the waya panda sees a fly as an enthusiastic cow. One cannot separate horses from lovely ants! Draped neatly on a hanger, the versatile blackberry comes from a protective cow. Some gentle hippopotamus are thought of simply as bears? The first lucky chimpanzee is, in its own way, a strawberry. The literature would have us believe that an affectionate giraffe is not but an alligator. The first diligent crocodile is, in its own way, a persimmon.',
        lat: -29.2035,
        lng: -42.0214,
    },
    {
        id: 21,
        name: 'East Jeanie',
        desc: 'She said: "Waking to the buzz of the alarm clock, a fig is a pear from the right perspective?" In her own words: "If this was somewhat unclear, the persimmon of a tiger becomes a lively eagle;" Shouting with happiness, the goat is a blueberry. Punctual nectarines show us how sharks can be deers.',
        lat: -86.7725,
        lng: 26.6931,
    },
    {
        id: 22,
        name: 'North Halliemouth',
        desc: 'Washing and polishing the car,one cannot separate blackberries from cultured eagles? In her own words: "Before watermelons, wolfs were only birds." Elephants are instinctive bananas. The first calm puppy is, in its own way, a kangaroo. Few can name an adventurous lemon that isn\'t an encouraging sheep. Authors often misinterpret the nectarine as an elated dog, when in actuality it feels more like a fabulous zebra. A blackberry is a dog from the right perspective. Quoting with intend: "Some bright cheetahs are thought of simply as owls."',
        lat: -59.3469,
        lng: -37.1787,
    },
    {
        id: 23,
        name: 'Funkside',
        desc: 'The sharks could be said to resemble jolly turtles? In his own words: "As far as he is concerned, their duck was, in this moment, an amiable frog." In her own words: "Incomprehensibilities of a amused rabbit and a persimmon made an orange selective." In his own words: "One cannot separate pineapples from brave rabbits?" Quoting with intend: "However, some fearless foxes are thought of simply as spiders?" She said: "Some energetic kittens are thought of simply as cows." We know that a grape is a chicken from the right perspective; In his own words: "Birds are eminent figs." Their nectarine was, in this moment, a determined lime.',
        lat: -14.065,
        lng: 168.1448,
    },
    {
        id: 24,
        name: 'Queenchester',
        desc: 'Some brave grapefruits are thought of simply as spiders? A funny crocodile without frogs is truly a goat of warm blueberries. In his own words: "Some assert that a credible blueberry is an ant of the mind?" One cannot separate currants from resourceful kumquats! Yelling: "Some resolute pineapples are thought of simply as flies." In ancient times goldfishes are comfortable dogs. Yelling: "Congratulations to the currant that won the boundless duck with a tiger." Few can name a silly monkey that isn\'t a unassuming crocodile! Few can name an inventive puppy that isn\'t a warm pomegranate? They were lost without the compassionate giraffe that composed their dog.',
        lat: 55.2666,
        lng: -11.0313,
    },
    {
        id: 25,
        name: 'Wunschfurt',
        desc: 'This is not to discredit the idea that the wonderful puppy reveals itself as a powerful prune to those who look? Of course, a melon can hardly be considered a tidy kiwi without also being a seal. Blueberries are courageous pomegranates. Yelling: "The chimpanzee is an ant." Extending this logic, a rabbit is a harmonious fox? Framed in a different way, some posit the thrifty octopus to be less than cheerful.',
        lat: -44.9507,
        lng: -170.9158,
    },
];

export const users = [
    {
        id: 1,
        username: 'Strawberry6',
        email: 'strawberry6@gmail.com',
    },
    {
        id: 2,
        username: 'Apple3',
        email: 'apple3@gmail.com',
    },
    {
        id: 3,
        username: 'Currant5',
        email: 'currant5@gmail.com',
    },
    {
        id: 4,
        username: 'Hamster4',
        email: 'hamster4@gmail.com',
    },
    {
        id: 5,
        username: 'Melon5',
        email: 'melon5@gmail.com',
    },
    {
        id: 6,
        username: 'Kitten7',
        email: 'kitten7@gmail.com',
    },
    {
        id: 7,
        username: 'Pineapple8',
        email: 'pineapple8@gmail.com',
    },
    {
        id: 8,
        username: 'Wolf9',
        email: 'wolf9@gmail.com',
    },
    {
        id: 9,
        username: 'Rat2',
        email: 'rat2@gmail.com',
    },
    {
        id: 10,
        username: 'Hippopotamus8',
        email: 'hippopotamus8@gmail.com',
    },
    {
        id: 11,
        username: 'Tangerine6',
        email: 'tangerine6@gmail.com',
    },
    {
        id: 12,
        username: 'Lemon1',
        email: 'lemon1@gmail.com',
    },
    {
        id: 13,
        username: 'Grape8',
        email: 'grape8@gmail.com',
    },
    {
        id: 14,
        username: 'Alligator4',
        email: 'alligator4@gmail.com',
    },
    {
        id: 15,
        username: 'Seal2',
        email: 'seal2@gmail.com',
    },
    {
        id: 16,
        username: 'Snake9',
        email: 'snake9@gmail.com',
    },
    {
        id: 17,
        username: 'Eagle1',
        email: 'eagle1@gmail.com',
    },
    {
        id: 18,
        username: 'Currant4',
        email: 'currant4@gmail.com',
    },
    {
        id: 19,
        username: 'Frog9',
        email: 'frog9@gmail.com',
    },
    {
        id: 20,
        username: 'Snail9',
        email: 'snail9@gmail.com',
    },
    {
        id: 21,
        username: 'Spider9',
        email: 'spider9@gmail.com',
    },
    {
        id: 22,
        username: 'Ant1',
        email: 'ant1@gmail.com',
    },
    {
        id: 23,
        username: 'Currant4',
        email: 'currant4@gmail.com',
    },
    {
        id: 24,
        username: 'Watermelon3',
        email: 'watermelon3@gmail.com',
    },
    {
        id: 25,
        username: 'Fig2',
        email: 'fig2@gmail.com',
    },
];

export const participants = [
    {
        id: 1,
        user_id: 1,
        event_id: 1,
    },
    {
        id: 2,
        user_id: 20,
        event_id: 8,
    },
    {
        id: 3,
        user_id: 11,
        event_id: 4,
    },
    {
        id: 4,
        user_id: 11,
        event_id: 15,
    },
    {
        id: 5,
        user_id: 19,
        event_id: 17,
    },
    {
        id: 6,
        user_id: 12,
        event_id: 16,
    },
    {
        id: 7,
        user_id: 12,
        event_id: 10,
    },
    {
        id: 8,
        user_id: 17,
        event_id: 17,
    },
    {
        id: 9,
        user_id: 15,
        event_id: 7,
    },
    {
        id: 10,
        user_id: 7,
        event_id: 18,
    },
    {
        id: 11,
        user_id: 14,
        event_id: 18,
    },
    {
        id: 12,
        user_id: 11,
        event_id: 14,
    },
    {
        id: 13,
        user_id: 8,
        event_id: 16,
    },
    {
        id: 14,
        user_id: 16,
        event_id: 11,
    },
    {
        id: 15,
        user_id: 14,
        event_id: 13,
    },
    {
        id: 16,
        user_id: 15,
        event_id: 20,
    },
    {
        id: 17,
        user_id: 13,
        event_id: 11,
    },
    {
        id: 18,
        user_id: 12,
        event_id: 14,
    },
    {
        id: 19,
        user_id: 6,
        event_id: 15,
    },
    {
        id: 20,
        user_id: 4,
        event_id: 14,
    },
    {
        id: 21,
        user_id: 17,
        event_id: 12,
    },
    {
        id: 22,
        user_id: 19,
        event_id: 10,
    },
    {
        id: 23,
        user_id: 15,
        event_id: 12,
    },
    {
        id: 24,
        user_id: 5,
        event_id: 15,
    },
    {
        id: 25,
        user_id: 10,
        event_id: 13,
    },
    {
        id: 26,
        user_id: 13,
        event_id: 14,
    },
    {
        id: 27,
        user_id: 12,
        event_id: 15,
    },
    {
        id: 28,
        user_id: 10,
        event_id: 14,
    },
    {
        id: 29,
        user_id: 17,
        event_id: 11,
    },
    {
        id: 30,
        user_id: 7,
        event_id: 18,
    },
    {
        id: 31,
        user_id: 14,
        event_id: 8,
    },
    {
        id: 32,
        user_id: 15,
        event_id: 5,
    },
    {
        id: 33,
        user_id: 14,
        event_id: 14,
    },
    {
        id: 34,
        user_id: 15,
        event_id: 12,
    },
    {
        id: 35,
        user_id: 6,
        event_id: 17,
    },
    {
        id: 36,
        user_id: 7,
        event_id: 12,
    },
    {
        id: 37,
        user_id: 16,
        event_id: 11,
    },
    {
        id: 38,
        user_id: 11,
        event_id: 14,
    },
    {
        id: 39,
        user_id: 16,
        event_id: 12,
    },
    {
        id: 40,
        user_id: 16,
        event_id: 12,
    },
    {
        id: 41,
        user_id: 16,
        event_id: 6,
    },
    {
        id: 42,
        user_id: 15,
        event_id: 8,
    },
    {
        id: 43,
        user_id: 7,
        event_id: 12,
    },
    {
        id: 44,
        user_id: 10,
        event_id: 12,
    },
    {
        id: 45,
        user_id: 13,
        event_id: 5,
    },
    {
        id: 46,
        user_id: 8,
        event_id: 12,
    },
    {
        id: 47,
        user_id: 10,
        event_id: 16,
    },
    {
        id: 48,
        user_id: 9,
        event_id: 13,
    },
    {
        id: 49,
        user_id: 25,
        event_id: 25,
    },
    {
        id: 50,
        user_id: 12,
        event_id: 12,
    },
];
